...
skip_gram_pairs = []
for sent in sentences:
    tokenised_sent = sent.lower().split()
    for i in range(1, len(tokenised_sent) - 1):
        word_context_pair = [[word2index_map[tokenised_sent[i - 1]], word2index_map[tokenised_sent[i + 1]]], word2index_map[tokenised_sent[i]]]
...
# Create Variables for the NCE (Noise-Contrastive Estimation) Loss function
    ...
    with open(os.path.join(LOG_DIR, 'metadata.tsv'), "w") as metadata:
        ...
        for step in range(1000):
            ...
            if step % 100 == 0:
                ...
                print("Loss at %d: %.5" % (step, loss_value))
